...
The data type `UUID` stores Universally Unique Identifiers (UUID) as defined by RFC 4122, ISO/IEC 9834-8:2005, and related standards. Ladybug follows PostgreSQL's implementation for the `UUID` format.
...
`NULL` is a special value to represent unknown data. Node or relationship properties and the result of expressions can be `NULL` in addition to the non-`NULL` domain of values they are allowed to take. For example, boolean expressions can be `true`, `false`, or `NULL`.
...
`TIMESTAMP` combines date and a time (hour, minute, second, and/or millisecond) and is formatted according to the ISO-8601 format (`YYYY-MM-DD hh:mm:ss[.zzzzzz][+-TT[:tt]]`), which specifies the date (`YYYY-MM-DD`), time (`hh:mm:ss[.zzzzzz]`) and a timezone offset `[+-TT[:tt]]`. Only the date section is mandatory. If the time is specified, then the millisecond `[.zzzzzz]` is optional. The default timezone is UTC, and Ladybug stores the timestamp based on the timezone offset relative to UTC.

Example:

```cypher
// Midnight in UTC-10
RETURN timestamp("1970-01-01 00:00:00-10") as x;
```
...
RETURN timestamp("1970-01-01 00:00:00.004666") as x;
```
...
`INTERVAL` consists of multiple date parts and represents the total time length of these date parts. Ladybug follows DuckDB's implementation for the interval format.
...
Functions that work on `STRUCT`s can be found in the struct functions documentation.
...
`MAP`s must have a single type for all keys, and a single type for all values. Additionally, keys of a `MAP` do not need to be `STRING`s like they do in a `STRUCT`.
...
Functions that work on map objects can be found in the map functions documentation.

## UNION

Similar to C++ `std::variant`, `UNION` is a nested data type that is capable of holding multiple alternative values with different types. The value under key `"tag"` is considered as the value being currently held by the `UNION`.
...
Functions that work on `UNION` data types can be found in the union functions documentation.
...
`BLOB` (Binary Large OBject) allows storage of an arbitrary binary object with up to 4KB in size in Ladybug. The database processes it as binary data because it has no knowledge as to what the underlying data represents (e.g. image, video).

Below is an example of how to create a blob object with 3 bytes (`\xBC`, `\xBD`, `\xBA`, `\xAA`):
...
Here's how to return a `NODE` column for a file `person.csv`:
...
Ladybug supports two list-like data types: (i) variable-length lists, simply called `LIST`, and (ii) fixed-length lists, called `ARRAY`. See the list and array functions documentation for more details.

## JSON

Using the native `JSON` type or the `json` extension, you can model and operate on properties as JSON natively via the `JSON` logical type, rather than as a string. It is recommended to store JSON data using the `JSON` type instead of `STRING`.
