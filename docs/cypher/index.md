...
Cypher is a high-level query language for the property graph data model. If you're coming from a SQL background, its syntax may seem familiar. Some common analogues between SQL and Cypher are listed below:
...
Ladybug implements openCypher's[^1] standard predicates and expressions. The following sections in this chapter cover all Cypher statements, clauses, expressions and functions implemented in Ladybug.
...
You can execute multiple query statements sequentially in the CLI, rather than executing them one by one. To run multi-line statements, simply end each valid statement you want to execute with a semicolon `;`. For example:
...
When you copy-paste the above blocks into the Ladybug CLI and press Enter, it will execute each block sequentially, so you don't have to send individual queries one at a time.
...
> **Note:** Cypher does not have an explicit `GROUP BY` like SQL does — instead, grouping is done implicitly depending on the combination of bound variables in the `RETURN` clause.
