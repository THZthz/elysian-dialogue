...
In this page, we list the syntactic features of Cypher as implemented in Ladybug. As described in the overview page, Cypher is a declarative graph query language, and Ladybug's implementation is based on openCypher.

## Parsing

### Encoding

The Cypher query parser looks for an input `STRING` that consists of ASCII or unicode characters. An example is shown below for creating and querying from a node table of German books.
...
To use reserved keywords in identifiers, you can escape them by encapsulating the identifier in backticks `` ` ``. For example, using the label `Return` for a node table requires escaping.
...
- Single line comments begin with `//` and continue up to the end of the line. They can be placed at the beginning, in the middle, or at the end of a query.
- Multi-line comments begin with `/*` and continue until they encounter `*/`. They can be used for comments that are too long to fit on one line.
...
- Table column names are case-insensitive. For example, `LOAD FROM 'myfile.csv' RETURN MyCoLuMn` will work even if the column name in the source is `mycolumn`.
...
See the prepared statements guide for more information on how to use parameters in Ladybug.
...
To use a reserved keyword as an identifier, you can escape it.
