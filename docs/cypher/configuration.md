...
The configuration of a Ladybug database or a connection to the database can be changed through a standalone `CALL` statement, described in this section. Different from the CALL clause, this statement for changing configuration cannot be used with other query clauses, such as `RETURN`.
