...
export const manageRelationship: Tool<typeof inputSchema> = {
  ...
  description: `
READ, UPSERT, or END relationships between nodes.
...
- **READ**: Look up relationships without writing Cypher. Modes (auto-detected from which fields are provided): source+target+type → specific details; source+type → all outgoing of that type; source only → all outgoing grouped by type; target+type → all incoming of that type; target only → all incoming grouped by type; source+target (no type) → all rels between them.
- **END**: Terminate a temporal relationship at a specified \`time\`. Only works on currently-active temporal relationships (\`valid_at IS NULL\`). Requires \`sourceMatch\`, \`targetMatch\`, \`relationshipType\`, and \`time\`.

Do not directly edit ABOUT_*/STARTED_AT/COMPLETED_AT/BRANCHES_TO — use \`${TOOL_NAMES.EDIT_NOTE}\` or \`${TOOL_NAMES.EDIT_PLOT}\` instead.

## Upsert behavior
- New: \`created_at\` auto-set to active scene time, \`valid_at\` starts NULL for temporal relationships.
- Existing: properties partially updated. JSON properties receive partial merge. \`created_at\` is immutable.
- Must be registered via \`${TOOL_NAMES.MANAGE_SCHEMA}\`. Discover schemas via \`${TOOL_NAMES.GET_CONTEXT}\` SCHEMA_DUMP.

## Temporal relationships
State-changing relationships (CHARACTER_AT, OBJECT_AT, CARRIED_BY, LOCATED_IN, HAS_DISPOSITION) are temporal — set \`valid_at\` to end, never delete.

## Spatial conventions
| relationship    | brief field describes                     |
|-----------------|-------------------------------------------|
| CHARACTER_AT    | spatial position ("hiding behind crates")  |
| OBJECT_AT       | spatial position ("on the table")          |
| LOCATED_IN      | access/containment ("trapdoor behind bar") |
| CARRIED_BY      | carry method ("concealed in a boot")       |

CHARACTER_AT → characters at location. OBJECT_AT → objects at location. CARRIED_BY (Object→Character) → on person. LOCATED_IN → sub-location nesting.
`.trim(),
  schema: inputSchema,
